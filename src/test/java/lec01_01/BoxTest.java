...
/**
 * BoxTest - это тест glassbox класса Box.
 *
 * Помните, что, как и контейнер для мячей, Коробка является контейнером для мячей, и вы
 * можно поместить lec01_01.Ball в коробку только один раз. После того, как вы поместите lec01_01.Ball в
 * поле, дальнейшие попытки сделать это завершатся неудачей, так как lec01_01.Ball находится
 * уже в коробке! Аналогично, вы не можете ожидать удаления lec01_01.Ball
 * из коробки, если она не находится внутри Коробки.
 *
 * Кроме того, Коробка отличается от балконтейнера тем, что она имеет только конечную
 * громкость. Когда шарики кладут в коробку, она заполняется. Один раз в коробке
 * заполнен, дальнейшие попытки положить мячи в коробку также завершатся неудачей.
 *
 * @see Ball
...
public class BoxTest {
    ...
    public static void setupBeforeTests() throws Exception {
        ...
        assertTrue("Test case error, you must test at least 1 lec01_01.Ball!",
                NUM_BALLS_TO_TEST>0);
    ...
    /** Test to check that Box.add(lec01_01.Ball) is implemented correctly */
    @Test
    public void testAdd() {
        box.clear();
        for (int i=0; i<BOX_CAPACITY; i++) {
            assertTrue("Box.add(lec01_01.Ball) failed to add a new lec01_01.Ball!",
                    box.add(b[i]));
            assertFalse("Box.add(lec01_01.Ball) seems to allow the same lec01_01.Ball to be added twice!",
                    box.add(b[i]));
        ...
        for (int i=BOX_CAPACITY; i<NUM_BALLS_TO_TEST; i++) {
            assertFalse("Box.add(lec01_01.Ball) allows a lec01_01.Ball to be added even though it is already full!",
                    box.add(b[i]));
        }
    }
    ...
    /**
     * Test to check that Box.remove(lec01_01.Ball) is implemented
    ...
    public void testRemove() {
        box.clear();
        assertFalse("Box.remove(lec01_01.Ball) should fail because box is empty, but it didn't!",
                box.remove(b[0]));
        for (int i=0; i<BOX_CAPACITY; i++) {
            box.clear();
            for (int j=0; j<i; j++) {
            ...
            for (int j=0; j<i; j++) {
                assertTrue("Box.remove(lec01_01.Ball) failed to remove a lec01_01.Ball that is supposed to be inside",
                        box.remove(b[j]));
                assertFalse("Box still contains a ball after it is supposed to have been removed!",
                        box.contains(b[j]));
            }
            for (int j=i; j<NUM_BALLS_TO_TEST; j++) {
                assertFalse("Box.remove(lec01_01.Ball) did not fail for a lec01_01.Ball that is not inside",
                        box.remove(b[j]));
            }
        }
    }
    ...
    /** Test to check that we can put a lec01_01.Ball into a Box */
    @Test
    ...
    public void testContains() {
        ...
        for (int i=0; i<BOX_CAPACITY; i++) {
            box.add(b[i]);
            assertTrue("Box does not contain a lec01_01.Ball that is supposed to be inside!",
                    box.contains(b[i]));
            for (int j=i+1; j<BOX_CAPACITY; j++) {
                assertFalse("Box seems to contain a lec01_01.Ball that is not inside!",
                        box.contains(b[j]));
            }
        }
    }
